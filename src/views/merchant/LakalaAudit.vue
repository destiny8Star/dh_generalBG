                    ...
                    <div class="main-inner" v-loading="loading">
                        ...
                        <template>
                            <el-table ref="multipleTable" :data="tableData" style="width: 100%;min-width: 780px;"
                                ...
                                <el-table-column prop="create_time" label="申请时间" :key="1">
                                    <template slot-scope="scope">
                                        <div>{{new Date(scope.row.create_time).format('yyyy-MM-dd hh:mm:ss')}}</div>
                                    </template>
                                </el-table-column>
                                <el-table-column prop="store_name" label="商户名" :key="2">
                                </el-table-column>
                                <el-table-column prop="real_name" label="姓名" :key="3">
                                </el-table-column>
                                <el-table-column prop="tel" label="手机号" :key="4">
                                </el-table-column>
                                <el-table-column prop="area" label="所属区域" :key="5">
                                    <template slot-scope="scope">
                                        <div>{{scope.row.area}}</div>
                                    </template>
                                </el-table-column>
                                <el-table-column prop="agent_name" label="推荐人(身份)" :key="6">
                                <!--<el-table-column prop="agent_name" label="所属服务商/分销商" :key="6">-->
                                    <template slot-scope="scope">
                                        <div>{{scope.row.agent_name}}</div>
                                    </template>
                                </el-table-column>
                                <el-table-column prop="status" label="审核状态" v-if="postData.status==3" :key="7">
                                    <template slot-scope="scope">
                                        <div>{{scope.row.status?scope.row.status:'--'}}</div>
                                    </template>
                                </el-table-column>
                                <el-table-column prop="audit_result" label="驳回原因" v-if='postData.status==2' :key="8">
                                </el-table-column>
                                <el-table-column fixed="right" label="操作" :key="9">
                                    <template slot-scope="scope">
                                        <el-button @click="handleClick(scope.row)" type="text">详情</el-button>
                                        <el-button v-show="(postData.status==3&&postData.lkl_mer_agree_status==1)||(postData.status==2&&!postData.lkl_mer_agree_status)" @click="statusClick(scope.row)"
                                                   type="text">刷新
                                        </el-button>
        ...
        data() {
            return {
                ...
                "postData": {
                    ...
                    "size": 10,//每页条数
                    "status": '3',//1审核成功,3审核中
                    'lkl_mer_agree_status':'0', //  0未提交商户协议1已提交商户协议
                    "store_name": "",//商户名
                    'agent_name': '', //所属分销商
                    "start_time": '',
                    "end_time": '',
        ...
        methods: {
            ...
            handleClick(data) {
                ...
                this.$router.push({
                    ...
                    query: {merchantId: data.id, status: this.postData.status, lklstatus: this.postData.lkl_mer_agree_status}
                })
            },
            /**
             * 删除
            ...
            //查询状态
            statusClick(data) {
                console.log(data, '查询状态');
                this.$axios.get('/admin/store/lkl/status?id=' + data.id)
                    .then(res => {
                        console.log(res);
            ...
            tabSelect(key, keyPath) {
                ...
                if(key == '3-0'){
                      ...
                      this.postData.lkl_mer_agree_status = 0
                }else if(key == '3-1'){
                    this.postData.status = 3
                    this.postData.lkl_mer_agree_status = 1
                }else{
                     this.postData.status = 2
                     this.postData.lkl_mer_agree_status = null
                }
                this.pagination.currentPage = this.postData.cursor = 1;
                this.getData()
            },
            //获取待审核和驳回数据
            getTop() {
                this.$axios.post('/admin/store/lkl/no')
                    .then(res => {
                        console.log("获取统计", res)
                        if (res.data.code == 200) {
                            this.tabList[0].num = res.data.data.un_submit_no
                            this.tabList[1].num = res.data.data.unrevised_no
                            this.tabList[2].num = res.data.data.refused_no
                        } else {
                            this.$message.error(res.data.message)
                        }
                    })
            ...
            getData() {
                this.loading = true;
                //初始化列表
                this.$axios.post('/admin/store/lkl/list', this.postData).then(res => {
                    this.loading = false;
                    console.log(res);
                    if (res.data.code === this.$webConfig.httpSuccessStatus) {
