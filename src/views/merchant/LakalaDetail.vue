            ...
            <div class="main-content" id="main-content" :style="{height: (getWinHeight-60)+'px'}">
                <div class="main-feature">
                    <div class="main-inner" :style="{minHeight: (getWinHeight-20)+'px'}" v-loading="loading">
                        ...
                        <div class="imgBoxs" v-if="lklstatus != 0">
                            <div class="imgBox" v-for="(ele,i) in other_pics" :key="i" @click="lookBigImg(ele)"
                                 :class="{bigAct:ele.flag}">
                                <img :src="ele" alt="">
                                <p>其它{{i+1}}</p>
                            </div>
                        </div>
                        <div class="form-list" style="margin-top: 30px;width: auto" v-if="lklstatus == 0">
                            <el-form label-position="left" label-width="140px">
                                ...
                                <el-form-item label="其它:" class="form-item">
                                    <el-upload
                                        ...
                                        <i class="el-icon-plus avatar-uploader-icon" @click="getToken('1')"></i>
                                    </el-upload>
    ...
    export default {
        ...
        methods: {
            ...
            handleAvatarSuccess(res, file,fileList) {
                console.log("file", res, file, fileList);
                this.other_pic = fileList;
                let url = '';
                if(this.onFN == '1'){
                    url = this.$webConfig.apiHosts + this.qn.key;
                    this.subImg.other_pics.push(url);
                }else {
                    this[this.onFN] = URL.createObjectURL(file.raw);
                    this.subImg[this.onFN] = this.$webConfig.apiHosts + this.qn.key
                }
                console.log("file", res, file, this.subImg.other_pics,this.other_pic)
            },
            /**
             * 获取token
             */
            ...
            handleRemove(file, fileList) {
                console.log(file, fileList,this.other_pic);
                let index;
                this.other_pic.forEach((ele,i) => {
                    console.log(ele,ele.uid);
                    if (ele.url == file.url) {
                        index = i;
                        this.subImg.other_pics.splice(i, 1);
                    }
                });
                console.log(index);
                this.other_pic.splice(index, 1);
                console.log(this.other_pic,this.subImg.other_pics);
            },
            /**
             * 图预览
             * @param file
            ...
            getData() {
                ...
                this.$axios.get('/admin/store/lkl/detail?id=' + this.id).then(res => {
                    ...
                    if (res.data.code === this.$webConfig.httpSuccessStatus) {
                        ...
                        this.lkl_mer_agree_pic = list.lkl_mer_agree_pic
                        this.lkl_mer_agree_back_pic = list.lkl_mer_agree_back_pic
                        this.other_pics = list.other_pics;
                        // this.subImg.other_pics = list.other_pics;
                        // this.other_pic = list.other_pics;
                    } else {
